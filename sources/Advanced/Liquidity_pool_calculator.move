module my_addrx::Liquidity_pool_calculator {
 /*
Liquidity Pool
...
Value2 = Multiply both USDT and Chaos Supply.
         MixSup: Coin1 * Coin2 = 998,712

Value3 = Determine the new supply of USDT after the swap.
         NewUSDT: Coin1 + 495 = 3,696

Value4 = Determine the new supply of Chaos after the swap.
         NewChaos: MixSup / (NewUSDT - fee) = 270

Value5 = Determine the amount of Chaos to transfer to user.
         Transfer = Coin2 - NewChaos = 42
...
*/    
    ...
    const Pool1_Chaos: u64 = 312;
    const Pool1_usdt: u64 = 3201;
    const Chaos_name: vector<u8> = b"N2D Rewards";
    ...
    fun get_supply(coin_symbol: u64): (u64, u64, vector<u8>) {
        if (coin_symbol == Chaos)
            return (Pool1_usdt, Pool1_Chaos, Chaos_name)
    ...
    fun calculate_swap(coin1: u64, coin2: u64, coin1_amt: u64): u64 {
        ...
        let new_Chaos = mix_supply / (new_usdt - fee);
        let receive = coin2 - new_Chaos;
        return receive
    }
...
Value5 = Determine the amount of Chaoss to transfer to user.
         Transfer = Coin2 - NewChaos = 42
